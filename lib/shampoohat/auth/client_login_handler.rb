...
module Shampoohat
  module Auth
    ...
    class ClientLoginHandler < Shampoohat::Auth::BaseHandler
      ACCOUNT_TYPE = 'GOOGLE'
      ...
      def generate_token(credentials)
        ...
        response = Shampoohat::Http.post_response(url, data, @config, headers)
      ...
      def handle_login_error(credentials, response, results)
        ...
        if 'CaptchaRequired'.eql?(results['Error'])
          ...
          raise Shampoohat::Errors::CaptchaRequiredError.new(results['Error'],
        ...
        raise Shampoohat::Errors::AuthError.new(error_message, error_str,
            results['Info'])
      end
