...
module Shampoohat
  class Api
    ...
    #
    # Throws:
    #  - Shampoohat::Errors::AuthError or derived if authetication error has
    #    occured
    #
    def authorize(parameters = {}, &block)
      ...
      if token.nil?
        ...
        rescue Shampoohat::Errors::OAuth2VerificationRequired => e
    ...
    def save_oauth2_token(token)
      raise Shampoohat::Errors::Error, "Can't save nil token" if token.nil?
      Shampoohat::Utils.save_oauth2_token(
    ...
    def create_auth_handler()
      ...
      return case auth_method
        ...
        when :OAUTH2
          ...
          Shampoohat::Auth::OAuth2Handler.new(
              @config,
        ...
        when :YAHOO, :CRITEO, :CRITEO_LOGIN, :NOTHING
        else
          raise Shampoohat::Errors::Error,
    ...
    def load_config(provided_config = nil)
      @config = (provided_config.nil?) ?
          Shampoohat::Config.new(
              File.join(ENV['HOME'], api_config.default_config_filename)) :
          Shampoohat::Config.new(provided_config)
      init_config()
    end
