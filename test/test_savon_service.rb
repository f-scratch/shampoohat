...
class StubService < Shampoohat::SavonService

  public :get_service_registry, :get_module

  def initialize(namespace, endpoint, version)
    @logger = Logger.new(STDERR)
    @config = Shampoohat::Config.new({:library => {:logger => @logger}})
...
class TestSavonService < Test::Unit::TestCase
  ...
  def test_initialize_abstract()
    assert_raises(NoMethodError) do
      Shampoohat::SavonService.new(nil, TEST_NAMESPACE, TEST_ENDPOINT,
          TEST_VERSION)
    end
